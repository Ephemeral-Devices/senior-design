...
class ThorStages(object):
    ...
    def device_search(self):
        ...
        if connected:
            ...
            self.num_of_devices = len(self.serial_numbers)
...
class InertialController(ThorStages):
    '''
    Class for the control of the Piezo Controller.
    ...
    def initialize(self):
        # Create the device
        self.device = KCubeInertialMotor.CreateDevice(self.serialNo)
        if self.device is None:
            print('Piezo Controller is a null object')
        else:
            print('Piezo Controller, ' + str(self.serialNo) + ', has been created')
        ...
        self.config = self.device.GetInertialMotorConfiguration(self.serialNo)
        # Create an instance of device setting.
        # Change the settings using this object and then use set
        # settings to load them into the device.

        self.mysettings = ThorlabsInertialMotorSettings.GetSettings(self.config)
        ...
        #if self.serial_reader is not 'Empty':
        #    self.reader = StrainReadr(self.serial_reader)
        #print("Nothing else should happen")
        #channel = InertialMotorStatus.MotorChannels.Channel1
    def setStepParameters(self,channel,rate,accel):
        self.mysettings.Drive.Channel(channel).StepRate = rate
        self.mysettings.Drive.Channel(channel).StepAcceleration = accel
    def zeroDevice(self, channel):
        self.device.SetPositionAs(channel, 0)
    def move(self, channel, position):
        try:
            self.device.MoveTo(channel, position, 60000)
        except Exception as e:
            print('Exception raised by move \n', e)
    def getPosition(self):
        return self.device.GetPosition(InertialMotorStatus.MotorChannels.Channel1)
